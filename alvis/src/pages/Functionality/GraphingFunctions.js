...
export function handleMoveFunc(lines, circles, e) {
  ...
  for (let line of tempCircle.connections) {
    ...
    let otherCircle = tempCircles.get(
      tempLine.connections.find((otherCircle) => otherCircle !== tempCircle.id)
    );
...
function deleteLine(newLines, newCircles, line) {
  ...
  for (let circle of tempLine.connections) {
    ...
    tempCircle.connections = tempCircle.connections.filter(
      (internalLine) => line !== internalLine
    );
