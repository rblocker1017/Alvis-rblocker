...
var str1 =
  "Step 1: Enter a string by clicking on the Reference String box with the format 'x, y, z...'   For example, the reference string of 1, 2, 3, 2, 3, 5, 4 is acceptable.";
var str2 =
  "Step 2: Enter a frame by clicking on the frame button and selecting a number.";
var str3 =
  "Step 3: Press the FIFO, LRU, or OPT buttons to select the algorithm.";
var str4 =
  "Step 4: Press the RESET button to remove the current reference string and Frame count.";
...
class PageReplacement extends Component {
  ...
  setFrames(e) {
    // takes in frames count and sets them to value of Frames
    this.setState({
      frames: e.target.value,
    });
  }
  runAlgorithm(e) {
    // takes in algorithm type, changes to it, and then runs algorithm
    this.setState({
  ...
  renderPageReplacement(newType) {
    // takes in new Algorithm type, and runs according to current values
    let bundle;
    switch (newType) {
      case "FIFO":
        bundle = Algorithms.fcfsPageReplacementFunc(
          this.state.input,
          this.state.frames
        );
        break;
      case "LRU":
        bundle = Algorithms.lruPageReplacementFunc(
          this.state.input,
          this.state.frames
        );
        break;
      case "OPT":
        bundle = Algorithms.optPageReplacementFunc(
          this.state.input,
          this.state.frames
        );
        break;
  ...
  reset() {
    // resets page replacement reference string and frame count, but doesn't remove page fault display
    this.setState({
      value: "",
        ...
        <td>
          {ans.column.map((page) => {
            if (JSON.stringify(page) !== '""') {
              // changes "" from boxes without a reference string value to blank for better visuals
              return (
                <tr>
                  <td
                    style={{
