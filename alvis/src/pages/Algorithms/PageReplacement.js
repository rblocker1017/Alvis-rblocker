export function fcfsPageReplacementFunc(pages, frames) {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    if (s.size < frames) {
      ...
      if (!s.has(pages[i])) {
        // enter here if page is not loaded into memory and all frames have already been assigned
        ...
        const orderedAnswer = prevStruct.map((page) => {
          // orderAnswer is an Array with values from the previous column
          if (s.has(page)) {
            // if the previous column is avaliable, grab it
            return page;
          } else {
...
export function lruPageReplacementFunc(pages, frames) {
  ...
  for (let i = 0; i < pages.length; i++) {
    // loops through maximum amount of frames first, like FCFS and OPT
    if (s.size < frames) {
      if (!s.has(pages[i])) {
        s.add(pages[i]);
        prevStruct = [...s];
