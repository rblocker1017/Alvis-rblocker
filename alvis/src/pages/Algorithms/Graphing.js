...
function hasPath(currentNode, end, lines, processedNodes) {
  ...
  lines.forEach((line) => {
    ...
    if (
      line.connections.includes(currentNode) &&
      !processedNodes.includes(currentNode)
    ) {
      ...
      const nextStart = line.connections.find(
        (circleId) => circleId !== currentNode
      );
      // OR the result with exists in order to pass the result through the end
      exists =
        exists ||
        hasPath(nextStart, end, lines, processedNodes.concat(currentNode));
    }
  });
  return exists;
}
    ...
    for (let lines of displayLines) {
      ...
      cyclical =
        ...
        isCyclical(
          node,
          newProc,
          displayLines.filter((disLines) => disLines !== lines)
        );
    }
  }
  return false;
...
export function kruskalAlgorithm(start, end, lines) {
  ...
  for (let i = 0; i < tempLines.length; i++) {
    if (
      !hasPath(
        tempLines[i].connections[0],
        tempLines[i].connections[1],
        displayLines,
        currentConnections
      )
    ) {
...
function primHelper(start, end, displayLines, tempLines, processedNodes) {
  ...
  for (let line of tempLines) {
    ...
    for (let node of processedNodes) {
      ...
      if (
        line.connections.includes(node) &&
        !processedNodes.includes(line.connections.find((id) => id !== node))
      ) {
 ...
 * @returns
 */
export function dijkstrasHelper(
  start,
  end,
  displayLines,
  tempLines,
