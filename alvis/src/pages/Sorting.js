...
export class Sorting extends Component {
  ...
  stepForward() {
    ...
    if (this.state.stepCount < this.state.arraysOfArrays.length - 1) {
      ...
      //sets step and swaps forward the bar or graph
      this.setState({
        stepCount: tempStep,
   ...
   */
  stepBack() {
    // If the step counter is greater than 0
    if (this.state.stepCount > 0) {
      let tempStep = this.state.stepCount - 1;
      //sets step and swaps back the bar or graph
      this.setState({
        stepCount: tempStep,
        newArray: this.state.arraysOfArrays[tempStep].data
          .split(",")
  ...
  //selectBar - sets clicked bar or graph to selected
  selectBar(d, i) {
  ...
  // updateGraph - updates the graph with new values after swapping
