...
class GraphingAlgorithm extends Component {
  ...
  insertValue() {
    ...
    if (
      ...
      let conId = JSON.stringify(
        [Number(this.state.selected), Number(this.state.otherInsert)].sort()
      ...
      newLines.set(
        conId,
        connectNode(toCircle, fromCircle, parseInt(this.state.input), conId)
      );
      this.setState({
        circles: newCircles,
        lines: newLines,
  ...
  addCircle(e) {
    ...
    if (this.state.step === -1) {
      ...
      this.setState({
        circles: Functions.addCircleFunc(
          this.state.circles,
          this.state.circleId,
          HEIGHT,
          WIDTH
  ...
  clearSelected() {
    ...
    if (this.state.selected !== null) {
      ...
      let tempNodes = Functions.clearSelectedFunc(
        this.state.lines,
        this.state.circles,
        this.state.selected
      );
  ...
  clearSteps() {
    ...
    this.setState({
      lines: Functions.clearStepsFunc(
        this.state.lines,
        this.state.algoArray,
        this.state.step
      ),
      step: -1,
    });
  ...
  stepForward() {
    ...
    if (
      this.state.step < this.state.algoArray.length - 1 &&
      this.state.startNode !== null
    ) {
      tempLines = Functions.stepForwardFunc(
        tempLines,
        this.state.algoArray,
        this.state.step
      );
  ...
  // @param: e - event listener
  handleMove(e) {
    const bundleMove = Functions.handleMoveFunc(
      this.state.lines,
      this.state.circles,
      e
    );
  ...
  selectNode(e) {
    ...
    isString
      ? (tempNodes = new Map(this.state.lines))
      : (tempNodes = new Map(this.state.circles));
  ...
  setStart() {
    ...
    if (
      ...
      this.state.selected === this.state.startNode ||
      this.state.selected === this.state.endNode
    ) {
      return;
    }
    ...
    this.setState({
      circles: Functions.setStartFunc(
        this.state.circles,
  ...
  finalConnect(e) {
    ...
    let conId = JSON.stringify(
      [Number(this.state.selected), Number(id)].sort()
    ); // tentative new id of the connection
  ...
  deleteNode() {
    ...
    if (this.state.selected !== null) {
      ...
      const deleteObject = Functions.deleteNodeFunc(
        this.state.circles,
        this.state.lines,
        this.state.selected,
        isString
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    //  opened
    if (
      nextState.changed !== this.state.type ||
      nextState.circles !== this.state.circles ||
      nextState.lines !== this.state.lines ||
          ...
          algorithms={[
            {
              ...
              time: "O(E log(V)), V = number of vertices",
              space: "O(V), V = number of vertices",
            },
            {
              name: "Dijkstras",
              func: this.changeAlgo,
              time: "O(V^2), V = number of vertices",
              space: "O(V), V = number of vertices",
            },
            {
              name: "Kruskal",
